...
App.controller('MainController', function ($scope, $http) {
    ...
    $scope.init = function () {
        $http.get('/api/1.0/profiles/' + ZZM.profileId)
        ...
        switch (mode) {
            case '':
                ...
                break;
            case 'system_id':
                $scope.fetchPlanetsResult($scope.profileId, $scope.systemId, 1, $scope.pageSize);
                break;
            case 'planet_id':
                $scope.fetchQuestionsResult($scope.profileId, $scope.planetId, 1, $scope.pageSize);
                break;
    ...
    $scope.fetchSystemResult = function (page, pageSize) {
        ...
        $http
                .get('/api/1.0/profiles/result/only-system?' + [
                    'page=' + page,
                    'page_size=' + pageSize,
                    'profile_id=' + $scope.profileId].join('&'))
                .success(function (data, status, headers, config) {
