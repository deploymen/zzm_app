...
Class AuthUserController extends Controller {
	...
	public function signUp() {
		...
		try {	
			...
			CampaignReferralHit::insert($newUser['user_id'] , $campaign);
	...
	public function invite() {
		...
		try {
			...
			$edmHtml = (string) view('emails.invitation', [
				...
				'link' => Config::get('app.ses_website_url') . '/sign-up/',
			]);
	...
	public function signUpApp() {
		...
		try {
			...
			LogHelper::LogToDatabase($ex->getMessage(), ['environment' => json_encode([
				'source' => 'AuthUserController > signUpApp',
				'inputs' => Request::all(),
	...
	public function handleProviderCallback() {
		...
		if($userExternalId){
			...
			$user = User::find($userExternalId->user_id);
            ...
            setcookie("current_user", json_encode([
            	'user' => [
					'id' => $user->id,
					'role' => $user->role,
					'name' => $user->name,
	...
	public function facebookSignUp(){
		...
		CampaignReferralHit::insert($user->id , $campaign);
